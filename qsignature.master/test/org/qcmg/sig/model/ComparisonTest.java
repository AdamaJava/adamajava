...
public class ComparisonTest {
	...
	public void testComparisonConstructor() {
		...
		Comparison c = new Comparison(F1, 0, F2, 0, 0, 0);
...
//		}	catch (IllegalArgumentException iae){}
	...
	public void testComparison() {
		Comparison c = new Comparison(F1, 0, F2, 0, 0, 1);
		assertEquals(0.0, c.getScore(), 0.00001);
		c = new Comparison(F1, 0, F2, 0, 10, 1);
		assertEquals(10.0, c.getScore(), 0.00001);
		c = new Comparison(F1, 0, F2, 0, 10, 20);
		assertEquals(0.5, c.getScore(), 0.00001);
		c = new Comparison(F1, 0, F2, 0, 10, 1000);
		assertEquals(0.01, c.getScore(), 0.00001);
		
		c = new Comparison("f1", 123456, "f2", 234567, 99999, 123456);
		...
		assertEquals("null:f1 (123456) vs null:f2 (234567) : " + (99999.0 / 123456) + ", 123456", c.toString());
		
	}
	
	@Test
	public void checkComparator() {
		Comparison c1 = new Comparison(F1, 0, F2, 0, 10, 100);
		Comparison c2 = new Comparison(F1, 0, F2, 0, 20, 100);
		assertEquals(-1, c1.compareTo(c2));
		
		c1 = new Comparison(F1, 0, F2, 0, 20, 100);
		assertEquals(0, c1.compareTo(c2));
		
		c1 = new Comparison(F1, 0, F2, 0, 200, 1000);
