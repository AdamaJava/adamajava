...
public class VcfHeaderUtilsTest {
	...
	public void mergeHeaders() {
		...
		h2.parseHeaderLine("##hello");		// not a filter, format, info or meta line
		VcfHeader.Record h2HelloRec = h2.getOtherRecords().get(0);
		assertEquals(false, VcfHeaderUtils.mergeHeaders(header, h2,false).getOtherRecords().contains(h2HelloRec));
		h2.addFilterLine("mySpecialFilter", "foo > bar");		// add a filter line
		VcfHeader.Record h2FilterRec = h2.getFilterRecords().get("mySpecialFilter");
		assertEquals(true, VcfHeaderUtils.mergeHeaders(header, h2,false).getFilterRecords().containsValue(h2FilterRec));
		h2.addFilterLine("mySpecialFilter", "foo == bar");		// add a filter line
		h2FilterRec = h2.getFilterRecords().get("mySpecialFilter");
		assertEquals(false, VcfHeaderUtils.mergeHeaders(header, h2,false).getFilterRecords().containsValue(h2FilterRec));
		assertEquals(true, VcfHeaderUtils.mergeHeaders(header, h2,true).getFilterRecords().containsValue(h2FilterRec));
		
		h2.addInfoLine("hotline", "1", "Flag", "This is my info line");		// add a filter line
		VcfHeader.Record h2InfoRec = h2.getInfoRecords().get("hotline");
		assertEquals(true, VcfHeaderUtils.mergeHeaders(header, h2,false).getInfoRecords().containsValue(h2InfoRec));
		h2.addInfoLine("hotline", "2", "Flag", "This is my info line");		// add a filter line
		h2InfoRec = h2.getInfoRecords().get("hotline");
		assertEquals(false, VcfHeaderUtils.mergeHeaders(header, h2,false).getInfoRecords().containsValue(h2InfoRec));
		assertEquals(true, VcfHeaderUtils.mergeHeaders(header, h2,true).getInfoRecords().containsValue(h2InfoRec));
		
		h2.addFormatLine("BAM", "1", "Flag", "This is my format line");		// add a filter line
		VcfHeader.Record h2FormatRec = h2.getFormatRecords().get("BAM");
		assertEquals(true, VcfHeaderUtils.mergeHeaders(header, h2,false).getFormatRecords().containsValue(h2FormatRec));
		h2.addFormatLine("BAM", "1", "String", "This is my format line");		// add a filter line
		h2FormatRec = h2.getFormatRecords().get("BAM");
		assertEquals(false, VcfHeaderUtils.mergeHeaders(header, h2,false).getFormatRecords().containsValue(h2FormatRec));
		assertEquals(true, VcfHeaderUtils.mergeHeaders(header, h2,true).getFormatRecords().containsValue(h2FormatRec));
	}
