...
public class VcfFormatFieldRecordTest {
    ...
    public void doesToStringWork() {
        ...
        try {
            ...
            assertNull(rec.getSampleColumnString());
    ...
    public void isMissingSample() {
        ...
        assertTrue(format.isMissingSample());
        format = new VcfFormatFieldRecord("GT", "ABC");
        assertFalse(format.isMissingSample());
    }
    ...
    public void getFieldTest() {
        ...
        assertNull(format.getField("ok"));
        assertEquals("1", format.getField("MR"));
        assertEquals(Constants.MISSING_DATA_STRING, format.getField("NNS"));
        assertEquals(Constants.MISSING_DATA_STRING, format.getField("GT"));
        format.setField("key", "value");
        assertEquals("value", format.getField("key"));
        format.setField("GT", null);
        assertEquals(Constants.MISSING_DATA_STRING, format.getField("GT"));
        format.setField("GT", "0/0");
        assertEquals("0/0", format.getField("GT"));
