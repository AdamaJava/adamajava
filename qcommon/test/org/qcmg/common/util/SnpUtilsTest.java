...
public class SnpUtilsTest {
    ...
    public void testIsClassA() {
        ...
        Assert.assertFalse(SnpUtils.isClassA(annotation));
        assertFalse(SnpUtils.isClassA("---")); // 3 dashes
        ...
        annotation = "123--";
        Assert.assertFalse(SnpUtils.isClassA(annotation));
        assertFalse(SnpUtils.isClassA("--"));
        assertTrue(SnpUtils.isClassA("PASS"));
        assertFalse(SnpUtils.isClassA("1PASS"));
        assertFalse(SnpUtils.isClassA("PASS1"));
        assertFalse(SnpUtils.isClassA("PASS--"));
        assertFalse(SnpUtils.isClassA("--PASS"));
    }
    ...
    public void getTotalBaseCountsCS() {
        ...
        assertEquals(17 + 17 + 2 + 10 + 8, SnpUtils.getTotalCountFromNucleotideString(bases, true));
    ...
    public void getCSDist() {
        assertTrue(SnpUtils.getCompoundSnpDistribution(null, 0).isEmpty());
        assertTrue(SnpUtils.getCompoundSnpDistribution("", 0).isEmpty());
        assertTrue(SnpUtils.getCompoundSnpDistribution("asdfsgahjsfkahs", 0).isEmpty());
        assertTrue(SnpUtils.getCompoundSnpDistribution("asdfsgahjsfkahs1232345", 0).isEmpty());

        String bases = "CA,17,17,C_,2,0,GG,10,8";
        Map<String, Integer> map = SnpUtils.getCompoundSnpDistribution(bases, 0);
        assertEquals(3, map.size());
        assertTrue(map.containsValue(34));
        assertTrue(map.containsValue(18));
        assertTrue(map.containsValue(2));
        assertTrue(map.containsKey("C_"));
        assertTrue(map.containsKey("GG"));
        assertTrue(map.containsKey("CA"));
    }

    @Test
    public void getCSDistMinCov() {
        assertTrue(SnpUtils.getCompoundSnpDistribution(null, 1).isEmpty());
        assertTrue(SnpUtils.getCompoundSnpDistribution("", 1).isEmpty());
        assertTrue(SnpUtils.getCompoundSnpDistribution("asdfsgahjsfkahs", 1).isEmpty());
        assertTrue(SnpUtils.getCompoundSnpDistribution("asdfsgahjsfkahs1232345", 1).isEmpty());
        ...
        assertTrue(map.containsKey("CA"));
        assertTrue(map.containsKey("GG"));
        assertTrue(map.containsKey("_C_"));
        map.clear();
    }
    ...
    public void testIsClassAIndel() {
        try {
            assertFalse(SnpUtils.isClassAIndel(null, -1));
            Assert.fail("Should have thrown an IAE");
        } catch (IllegalArgumentException iae) {
        }
        try {
            assertFalse(SnpUtils.isClassAIndel("", -1));
            Assert.fail("Should have thrown an IAE");
        } catch (IllegalArgumentException iae) {
        }

        // PASS
        assertFalse(SnpUtils.isClassAIndel("pass", -1));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.PASS, -1));

        //INDEL_HOM_ADJ
        try {
            assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_ADJ, 3));
            Assert.fail("Should have thrown an IAE");
        } catch (IllegalArgumentException iae) {
        }
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_ADJ + 9, 3));
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_ADJ + 8, 3));
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_ADJ + 7, 3));
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_ADJ + 4, 3));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_ADJ + 3, 3));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_ADJ + 1, 3));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_ADJ + 0, 3));

        //INDEL_HOM_CON
        try {
            assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_CON, 3));
            Assert.fail("Should have thrown an IAE");
        } catch (IllegalArgumentException iae) {
        }
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_CON + 9, 3));
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_CON + 8, 3));
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_CON + 7, 3));
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_CON + 4, 3));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_CON + 3, 3));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_CON + 1, 3));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_CON + 0, 3));

        //INDEL_HOM_EMB
        try {
            assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_EMB, 3));
            Assert.fail("Should have thrown an IAE");
        } catch (IllegalArgumentException iae) {
        }
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_EMB + 9, 3));
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_EMB + 8, 3));
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_EMB + 7, 3));
        assertFalse(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_EMB + 4, 3));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_EMB + 3, 3));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_EMB + 1, 3));
        assertTrue(SnpUtils.isClassAIndel(SnpUtils.INDEL_HOM_EMB + 0, 3));
    }
    ...
    public void testIsClassAOrB() {
        ...
        annotation = "- -";    // 2 dashes with space between them
        Assert.assertFalse(SnpUtils.isClassAorB(annotation));
        assertFalse(SnpUtils.isClassAorB("---"));
        annotation = " --";    // 2 dashes with leading space
        Assert.assertFalse(SnpUtils.isClassAorB(annotation));
        annotation = "-- ";    // 2 dashes with trailing space
        Assert.assertFalse(SnpUtils.isClassAorB(annotation));
        ...
        Assert.assertFalse(SnpUtils.isClassAorB(annotation));
        assertFalse(SnpUtils.isClassAorB("--"));
        assertTrue(SnpUtils.isClassAorB("PASS"));
        assertTrue(SnpUtils.isClassAorB(SnpUtils.PASS));

        Assert.assertFalse(SnpUtils.isClassAorB(SnpUtils.LESS_THAN_8_READS_NORMAL));
        assertTrue(SnpUtils.isClassAorB(SnpUtils.LESS_THAN_12_READS_NORMAL));
        assertFalse(SnpUtils.isClassAorB("; " + SnpUtils.MUTATION_IN_UNFILTERED_NORMAL));
        assertTrue(SnpUtils.isClassAorB(SnpUtils.MUTATION_IN_UNFILTERED_NORMAL));
        assertTrue(SnpUtils.isClassAorB(SnpUtils.LESS_THAN_3_READS_NORMAL));
        assertFalse(SnpUtils.isClassAorB("; " + SnpUtils.MUTATION_IN_UNFILTERED_NORMAL));
        assertTrue(SnpUtils.isClassAorB(SnpUtils.LESS_THAN_12_READS_NORMAL_AND_UNFILTERED));
        assertTrue(SnpUtils.isClassAorB(SnpUtils.LESS_THAN_3_READS_NORMAL_AND_UNFILTERED));
    ...
    public void testIsAnnotationAlone() {
        assertFalse(SnpUtils.isAnnotationAlone("PASS;Low_complexity::G-rich;NNS;", SnpUtils.INDEL_LOW_COMPLEXITY));
        assertFalse(SnpUtils.isAnnotationAlone("PASS;Satellite::HSATII;HOMCON_10;", SnpUtils.INDEL_SATELLITE));
        assertFalse(SnpUtils.isAnnotationAlone("PASS;Simple_repeat::(GAAA)n;NNS;", SnpUtils.INDEL_SIMPLE_REPEAT));
        assertFalse(SnpUtils.isAnnotationAlone("PASS;Simple_repeat::(TC)n;;Simple_repeat::(TG)n;MIN;HOMCON_8;", SnpUtils.INDEL_SIMPLE_REPEAT));
        assertFalse(SnpUtils.isAnnotationAlone("PASS", SnpUtils.INDEL_SIMPLE_REPEAT));
        assertTrue(SnpUtils.isAnnotationAlone("PASS", SnpUtils.PASS));
        assertTrue(SnpUtils.isAnnotationAlone("PASS;PASS", SnpUtils.PASS));
        assertTrue(SnpUtils.isAnnotationAlone("PASS;" + SnpUtils.INDEL_LOW_COMPLEXITY, SnpUtils.INDEL_LOW_COMPLEXITY));
    }
    ...
    public void testIndels() {
        assertFalse(SnpUtils.isClassAIndel("PASS;NNS;HCOVT;HCOVN;MIN;HOMCON_7", 3));
    }
