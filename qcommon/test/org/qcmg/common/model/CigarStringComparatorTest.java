...
public class CigarStringComparatorTest {
	...
	public void testCompare(){
		TreeMap<String, Integer> map = new TreeMap<>(new CigarStringComparator());
		map.put("2H",1);
		map.put("3H",1);
		map.put("10H",1);
		map.put("11H",1);
        ...
        Assert.assertEquals("35C", map.firstKey());
        Assert.assertEquals("200M", map.lastKey());
	}
	
	@Test
	public void testEmptyCigarString() {
		Map<String, Integer> map = new TreeMap<>(new CigarStringComparator());
		map.put("1D", 1);
		try {
			map.put("", 1);
