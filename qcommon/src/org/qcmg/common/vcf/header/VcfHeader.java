...
public class VcfHeader implements Iterable<VcfHeaderRecord> {
    ...
    //store all record with format ##key=<ID=id,...>, eg. qPG, Format, info and filter
    private final List<VcfHeaderRecord> idRecords = new ArrayList<>();

    //store all record follow patter ##key=value
    private final List<VcfHeaderRecord> metaRecords = new ArrayList<>();
    ...
    private <T extends VcfHeaderRecord> void replaceRecord(List<T> stack, T record, boolean isReplace) {
        ...
        while (ite.hasNext()) {
            T re = ite.next();
            boolean isSameId = (record.getId() == null && re.getId() == null) || (re.getId() != null && re.getId().equals(record.getId()));
     ...
     * @param isReplace: the existing record will be replaced if set to true; Otherwise if set to false, id of record must be unique,
     *                   so the existing one will be kept and input one will be discarded; line without id is allowed for multi entry, so both existing one and input line will be kept.
     */
     ...
     *
     * @param line:a     vcf Meta-information line, must follow  ##key=value pattern, except header line start with #CHROM.
     * @param isReplace: the existing header line will be replaced if set to true; Otherwise if set to false, header line contains id must be unique,
     *                   so the existing one will be kept and input one will be discarded; line without id is allowed for multi entry, so both existing one and input line will be kept.
     */
    ...
    /**
     * create a new vcf header by reading whole list of string
     *
    ...
    /**
     * Retrieves a list of records from the given stack that match the specified key.
     *
     * @param <T>   The type of the records.
     * @param stack The list of records to search.
     * @param key   The key to match the records against.
     * @return A list of records that match the specified key, or an empty list if no matches are found.
     */
    private <T extends VcfHeaderRecord> List<T> getRecords(List<T> stack, String key) {
        return stack.stream().filter(r -> r.getMetaKey().equals(key)).sorted().collect(Collectors.toList());
    }
     ...
     * @return a list of vcf header record follow pattern ##FILTER=<ID=id,...>
     */
    public List<VcfHeaderRecord> getContigRecords() {
        return getRecords(idRecords, VcfHeaderUtils.HEADER_LINE_CONTIG);
    }
    ...
    public Iterator<VcfHeaderRecord> iterator() {
        ...
        getFilterRecords().stream().sorted().forEach(records::add);
        getInfoRecords().stream().sorted().forEach(records::add);
        getFormatRecords().stream().sorted().forEach(records::add);
        ...
        idRecords.stream().filter(r -> !r.getMetaKey().equals(VcfHeaderUtils.HEADER_LINE_FILTER) &&
                ...
                !r.getMetaKey().equals(VcfHeaderUtils.HEADER_LINE_INFO)).sorted().forEach(records::add);
