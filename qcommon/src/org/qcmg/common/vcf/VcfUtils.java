...
public class VcfUtils {
    ...
    public static List<String> convertFFMapToList(Map<String, String[]> ffm, String[] orderedHeaders) {
        ...
        ffl.add(new StringBuilder(String.join(Constants.COLON_STRING, header)));
    ...
    public static String getGenotypeFromGATKVCFRecord(VcfRecord rec) {
        ...
        if (!formats.get(0).startsWith("GT")) {
            throw new IllegalArgumentException("VCFRecord passed to VcfUtils.getGenotypeFromGATKVCFRecord contains a format field that is malformed: rec: [" + rec + "]");
        }
    ...
    public static Map<String, String[]> getFormatFieldsAsMap(List<String> ff) {
        if (null != ff && !ff.isEmpty()) {
            int ffSize = ff.size();
            String[] headerFields = TabTokenizer.tokenize(ff.getFirst(), Constants.COLON);
    ...
    /**
     * it adjust the end position of cp if it is not corrected
     *
     ...
     * @param alt: String of allele base can be null
     * @return a vcfRecord
     */
    ...
    /**
     * Resets the allele of the given VcfRecord with the provided alternate allele.
     *
     * @param re  The VcfRecord to reset the allele for.
     * @param alt The alternate allele to set.
     * @return A new VcfRecord with the allele reset.
     */
    public static VcfRecord resetAllele(VcfRecord re, String alt) {
    ...
    public static void addMissingDataToFormatFields(VcfRecord vcf, int position, int count) {
        ...
        if (null != formatFields && !formatFields.isEmpty()) {
            ...
            String formatColumns = formatFields.getFirst();
            //split by ":"
            int noOfColumns = formatColumns.split(Constants.COLON_STRING).length;
            for (int j = 0; j < count; j++) {
                formatFields.add(position, (formatColumns.startsWith(VcfHeaderUtils.FORMAT_GENOTYPE) ? Constants.MISSING_GT : Constants.MISSING_DATA_STRING) + ":.".repeat(Math.max(0, noOfColumns - 1)));
            }
    ...
    public static void addAdditionalSamplesToFormatField(VcfRecord vcf, List<String> additionalSampleFormatFields) {
        ...
        if (additionalSampleFormatFields.size() == 2) {
            ...
            for (int i = 1; i < additionalSampleFormatFields.size(); i++) {
                addAdditionalSampleToFormatField(vcf, Arrays.asList(additionalSampleFormatFields.getFirst(), additionalSampleFormatFields.get(i)));
            }
        }
    }
    ...
    public static void addAdditionalSampleToFormatField(VcfRecord vcf, List<String> additionalSampleFormatFields) {
        if (null != additionalSampleFormatFields && additionalSampleFormatFields.size() > 1) {
            ...
            List<String[]> additionalSampleFormatFieldsParams = additionalSampleFormatFields.stream().map(s -> s.split(Constants.COLON_STRING)).toList();
            List<String> existingFF = vcf.getFormatFields();

            if (existingFF.isEmpty()) {
                throw new IllegalArgumentException("can't append additionalSampleFormatField to vcf with null on format column: " + vcf);
            }
            ...
            String[] newSampleHeaders = additionalSampleFormatFieldsParams.getFirst();
             ...
             */
            List<String> keysToBeefUp = ffMap.entrySet().stream().filter(e -> e.getValue().length < updatedNumberOfSamples).map(Map.Entry::getKey).toList();
        ...
        if (null != additionalFormatFields && !additionalFormatFields.isEmpty()) {
            ...
            if (null == existingFF || existingFF.isEmpty()) {
                ...
                if (existingFF.size() != additionalFormatFields.size()) {
                    ...
                    logger.info("vcf: " + vcf + ", additional fields: " + Arrays.deepToString(additionalFormatFields.toArray(new String[]{})));

                } else {
                    // need to check each element to see if it already exists...
                    final String[] formatFieldAttributes = additionalFormatFields.getFirst().split(COLON_STRING);
                    final String existingFieldAttributes = existingFF.getFirst();
                        ...
                        if (existingFieldAttributes.contains(s)) {
                            if (appendInfo) {
                                ...
                                for (int j = 1; j < additionalFormatFields.size(); j++) {
                                    ...
                                    // re-insert into vcf
                                    existingFF.set(j, String.join(Constants.COLON_STRING, existingArray));
                                }
                            }
                        } else {
                            // add this one
    ...
    public static String calculateAD(String oldAD, String newGt, String oldGt) {
        ...
        return String.join(Constants.COMMA_STRING, newADArray);
    }
    ...
    public static Optional<VcfRecord> mergeGATKVcfRecs(VcfRecord control, VcfRecord test) {
        ...
        if (control.getAlt().equals(test.getAlt())) {
            ...
            /*
             * need to update PL entry in format field if it exists to cater for tri-allelic values - just put in 0s for the additional allele
             */
    ...
    public static void prepareGATKVcfForMerge(VcfRecord v) {
        /*
     ...
     * If it's not, the supplied annotation is appended to the existing annotation(s)
     * <p>
    ...
    public static boolean isRecordAPass(Map<String, String[]> ffMap) {
        ...
        long somCount = null != infoArr ? Arrays.stream(infoArr).filter(f -> f.contains(VcfHeaderUtils.INFO_SOMATIC)).count() : 0;
        /*
    ...
    public static boolean isRecordAPass(Map<String, String[]> ffMap, VcfFileMeta meta, String infoField, boolean compoundSnp) {
         ...
         *
         * If it is a compound snp and
         */
        String[] infoArr = ffMap.get(VcfHeaderUtils.FORMAT_INFO);
        long somaticCount = null != infoArr ? Arrays.stream(infoArr).filter(f -> f.contains(VcfHeaderUtils.INFO_SOMATIC)).count() : 0;
        if (compoundSnp) {

            /*
             * If it is a compound snp, there is an assumption that this will have only been called by
    ...
    public static boolean isRecordAPassOldSkool(VcfRecord v) {
        ...
        long passCount = Arrays.stream(filterArr).filter(VcfHeaderUtils.FILTER_PASS::equals).count();
    ...
    public static int getSomaticCountFromFormatInfoFields(String[] infos) {
        if (null != infos && infos.length > 0) {
            return (int) Arrays.stream(infos).filter(f -> null != f && f.contains(VcfHeaderUtils.INFO_SOMATIC)).count();
        }
        return 0;
    }
    ...
    public static String getConfidence(VcfRecord rec) {
        ...
        return info.substring(index + CONF_LENGTH, scIndex > -1 ? scIndex : info.length());
    }
