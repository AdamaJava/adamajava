...
public class ReportBuilder {
	...
	public static QLogger log = QLoggerFactory.getLogger(ReportBuilder.class);
	...
	private static void addEntryToSummaryMap(Element reportElement, String elementName, String mapEntryName, Map<String, Map<String, AtomicLong>> summaryMap) {
		...
		if (null != element) {
			...
			for (Entry<String, AtomicLong> entry : sourceMap.entrySet()) {
				...
				Map<String, AtomicLong> map = summaryMap.computeIfAbsent(mapEntryName, k -> new HashMap<>());
				map.put(entry.getKey(), entry.getValue());
			}
		} else {
			log.warn("null " + elementName  + " element");
		}
	}
	...
	private static void createFaSummary(Element reportElement, Report report) {
		...
		// setup parent tab
		report.addTab(addTop100Chart(reportElement, "KMERS", "Kmer", "kmer", "Top 100 6-mers seen in reference genome", 100));
	}
	...
	private static void createFastqSummary(Element reportElement, Report report) {
		...
		ChartTab indexesTab = addTop100Chart(reportElement, "INDEXES", "Index", "index", "Top 50 indexes seen in fastq sequencing reads", 50);
		if (null != indexesTab) {
	...
	private static ChartTab addTop100Chart(Element reportElement, String nodeName, String charTitle, String chartId, String description, int number) {
	...
	private static void createFLAGS(Element reportElement, Report report) {
		...
		Map<String, AtomicLong> flagsKeyChange = new LinkedHashMap<>();
		for (Entry<String, AtomicLong> entry : flags.entrySet()) {
			String[] flagStringArray = entry.getKey().split(", ");
			flagsKeyChange.put((flagStringArray.length > 1 ? flagStringArray[1] + ", ": "")  + flagStringArray[0], entry.getValue());
		}

		flagTab.addChild(getChartTabFromMap( "Flag", "fl", HTMLReportUtils.BAR_CHART, flagsKeyChange) );

		// duplicates
		final Map<String, String> dupMap = new HashMap<>();
		...
		flagTab.addChild(getChartTabFromMap(DUPLICATE, "fld", HTMLReportUtils.PIE_CHART, duplicateFlags));
		// vendor check
		final Map<String, String> vendorMap = new HashMap<>();
		...
		flagTab.addChild(getChartTabFromMap("Vendor Check", "flf", HTMLReportUtils.PIE_CHART, failedFlags));
		// first and second
		final Map<String, String> firstSecondMap = new HashMap<>();
		...
		flagTab.addChild(getChartTabFromMap("Read in Pair", "flfs", HTMLReportUtils.PIE_CHART, firstSecondFlags));

		final Map<String, String> mappedMap = new HashMap<>();
		...
		flagTab.addChild(getChartTabFromMap(UNMAPPED, "flm", HTMLReportUtils.PIE_CHART, mappedFlags));
		...
		flagTab.addChild(getChartTabFromMap("Primary", "fls", HTMLReportUtils.PIE_CHART, primaryFlags));
	...
	//coverage for each chromosome and readGroup
	private static void createRNM(Element reportElement, Report report) {
		...
		for (int i = 0 , length = nlTop.getLength() ; i < length ; i++) {
			...
			//viral have 6000 contig's lots of them big then 1M, it causes html can't show well on browsers
			//chrMT is small but special
			if (contigLength > 50 * 1000 * 1000 || chromosome.toUpperCase().startsWith("CHR")){
		...
		chromos.sort(new ReferenceNameComparator());
		...
		assert readGroups != null;
		readGroups.remove(0);

		for (Entry<String, TreeMap<Integer, AtomicLongArray>> map : rgCountsMaps.entrySet()) {
			String keyWithOutPeriods = map.getKey().replace(".","");
			TreeMap<Integer, AtomicLongArray> notTotalMap = new TreeMap<>();
			...
			String extraOption = String.format(", vAxis: { viewWindowMode:'explicit', viewWindow:{  max: %d }}, fontSize:12, legend: {position: 'right', textStyle: {color: 'blue'}}, crosshair: {trigger: 'both'}, lineWidth: 2", cellingValue);
	...
	private static ChartTab createKmersTab(Element mersElement,  String tabTitle ){
		...
		Map<Integer, AtomicLongArray> map = new TreeMap<>();
	...
	private static ChartTab createCigarChartTab(Element element) {
		...
		final Map<String, AtomicLong> cigarsD = new TreeMap<>(new CigarStringComparator());
		final Map<String, AtomicLong> cigarsI = new TreeMap<>(new CigarStringComparator());
		final Map<String, AtomicLong> cigarsH = new TreeMap<>(new CigarStringComparator());
		final Map<String, AtomicLong> cigarsS = new TreeMap<>(new CigarStringComparator());
	...
	private static ChartTab createCoverageChartTab(Element element) {
		...
		if ( ! tallys.isEmpty()) {
			...
			TreeMap<Integer, AtomicLong> tallysTM = new TreeMap<>(tallys);
			final int maxCoverageValue = tallysTM.lastKey();
			...
			while (true) {
				...
				if (multiplier < maxCoverageValue) {
					// set multiplier to the next level
					multiplier *= 10;
			...
			// cumulative
			Map<Integer, AtomicLong> cumulativeTallys = new TreeMap<>();
			long count = 0;
			int i = 0;
	...
	private static ChartTab createMatrixChartTab(Element element, String title, String idSuffix) {
		...
		Map<MAPQMiniMatrix, AtomicLong> tallys = generateMatrixCollection(element);
	...
	private static Map<Integer, AtomicLong> createRangeTallyMap(Element element) {
		Map<Integer, AtomicLong> map = new TreeMap<>();
	...
	private static ChartTab generateTallyChartTab(Element element, String name,
	...
	private static <T> ChartTab getChartTabFromMap(String name, String id,
		...
		return getChartTabFromMap(name, id, chartType, true, cycleCount, false);
	}
	...
	private static <T> Map<T, AtomicLong> getMapFromElement(Element element) {
		final Map<T, AtomicLong> cycleCount = new LinkedHashMap<>();
	...
	private static Map<MAPQMiniMatrix, AtomicLong> generateMatrixCollection(Element tabElement) {
		...
		final Map<MAPQMiniMatrix, AtomicLong> lengths = new LinkedHashMap<>();

		if (null != tabElement) {
			NodeList nl = tabElement.getElementsByTagName("ValueTally");
			//FIXME hack to get around the "NEW" tag added to lengths
			if (nl.getLength() == 0) {
				nl = tabElement.getElementsByTagName("ValueTally" + "NEW");
			}
			Element nameElement = (Element) nl.item(0);
	...
	private static <T> List<ChartTab> createCyclesTabFromCollections(boolean showMainTab, String mainTabName,
		...
		List<ChartTab> tabs = new ArrayList<>();
	...
	private static Map<String, String>  createRgMap( Element summaryElement, Element isizeElement ) {
		...
		int rgNum = readsChildren.getLength();
		...
		for (  String rg : totalReadsMap.keySet()) {
			...
			try{
				float lost =  Float.parseFloat(lostMap.get(rg).replace("%", "").trim());
