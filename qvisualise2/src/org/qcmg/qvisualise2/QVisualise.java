...
import org.qcmg.qvisualise2.report.HTMLReportGenerator;
import org.qcmg.qvisualise2.report.Report;
...
public class QVisualise {
	...
	protected int engage() throws Exception {
		...
		if (null != doc) {
			...
			if (reports.isEmpty()) {
				...
				try { out.write(html); } finally { out.close(); }
			}
		} else {
	...
	public int setup(String args[]) throws Exception{
		...
		if (options.hasHelpOption()) {
			...
			System.err.println( messages.getMessage("INSUFFICIENT_ARGUMENTS"));
			System.err.println(messages.getUsage());
			...
			if ( ! FileUtils.isFileTypeValid(inputFile, XML_EXTENTION))
				throw new Exception( messages.getMessage("UNSUPPORTED_FILE_TYPE", inputFile));
			if ( ! FileUtils.isFileTypeValid(outputFile, HTML_EXTENTION))
				throw new Exception( messages.getMessage("UNSUPPORTED_FILE_TYPE", outputFile));
			
			// now check that we can read the input and write to the output
			if ( ! FileUtils.canFileBeRead(inputFile))
				throw new Exception( messages.getMessage("CANT_READ_INPUT_FILE", inputFile));
			if ( ! FileUtils.canFileBeWrittenTo(outputFile))
				throw new Exception( messages.getMessage("CANT_WRITE_TO_OUTPUT_FILE", outputFile));
			
			// don't like empty input files
			if (FileUtils.isFileEmpty(inputFile))
				throw new Exception( messages.getMessage("EMPTY_INPUT_FILE", inputFile));
