...
public class AnnotationSourceTSVTest {
	...
	@Test
	public void fieldOrderPreservedFromUserRequest() {
		/*
		 * header has fields in order: alpha, beta, gamma
		 * user requests them in reverse: gamma,beta,alpha
		 * output should match user-requested order, not header order
		 */
		String header = "alpha\tbeta\tgamma";
		String[] record = new String[]{"val_alpha", "val_beta", "val_gamma"};

		Map<String, Integer> fieldsAndPositions = AnnotationSourceTSV.getHeaderNameAndPositions("gamma,beta,alpha", header);
		assertEquals(2, fieldsAndPositions.get("gamma").intValue());
		assertEquals(1, fieldsAndPositions.get("beta").intValue());
		assertEquals(0, fieldsAndPositions.get("alpha").intValue());

		// build arrays the same way the constructor does (using split order, not keyset order)
		String[] requestedFields = "gamma,beta,alpha".split(",");
		int[] positions = new int[requestedFields.length];
		for (int i = 0; i < requestedFields.length; i++) {
			positions[i] = fieldsAndPositions.get(requestedFields[i]);
		}
		// output must be in user-requested order: gamma, beta, alpha
		assertEquals("gamma=val_gamma\tbeta=val_beta\talpha=val_alpha",
				AnnotationSourceTSV.extractFieldsFromRecord(record, requestedFields, positions));
	}
