package QCMG::SamTools::Bam::AlignWrapper;

# $Id: AlignWrapper.pm 1290 2011-10-19 03:43:58Z l.fink $

=head1 NAME

QCMG::SamTools::Bam::AlignWrapper -- Add high-level methods to QCMG::SamTools::Bam::Alignment

=head1 SYNOPSIS

See L<QCMG::SamTools::Bam::Alignment>.

=head1 DESCRIPTION

This is a wrapper around QCMG::SamTools::Bam::Alignment that adds the
following high-level methods. These are described in detail in
L<QCMG::SamTools::Bam::Alignment/High-level QCMG::SamTools::Bam::Alignment methods>.
 ...
 seq()                 return QCMG::SamTools::PrimarySeq object for reference sequence
...
sub split_splices {
	...
	if ($operation eq 'N') {
	    ...
	    my $f = QCMG::SamTools::Bam::SplitAlignmentPart->new(-name   => $self->display_name,
							  -start  => $s,
							  -end    => $e,
							  -seq_id => $self->seq_id,
							  -strand => +1,
...
sub seq      {
    my $self   = shift;
    my $dna    = $self->dna;
    return QCMG::SamTools::PrimarySeq->new(-seq => $dna,
				-id  => $self->seq_id);
}
...
sub subseq {
    ...
    my $dna = $self->dna;
    return QCMG::SamTools::PrimarySeq->new(-seq=>substr($dna,
					     $start-1,
					     $end-$start+1)
				);
}
...
package QCMG::SamTools::Bam::SplitAlignmentPart;
...
sub hit {
    my $self = shift;
    my $d    = $self->{hit};
    $self->{hit} = QCMG::SamTools::SeqFeature->new(@_) if @_;
    return $d;
}
...
sub QCMG::SamTools::SeqFeature::subseq {
    my $self = shift;
    my ($start,$end) = @_;
    $start = 1 if $start < 1;
    $end   = $self->high if $end > $self->high;
    return QCMG::SamTools::PrimarySeq->new(-seq=>substr($self->dna,
					     $start-1,
					     $end-$start+1)
				);
}
