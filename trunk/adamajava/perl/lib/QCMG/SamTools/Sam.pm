...
QCMG::SamTools::Sam -- Read SAM/BAM database files
...
=head1 SYNOPSIS
 ...
 # high level API
 my $sam = QCMG::SamTools::Sam->new(-bam  =>"data/ex1.bam",
                             -fasta=>"data/ex1.fa",
			     );
 ...
 # low level API
 my $bam          = QCMG::SamTools::Bam->open('/path/to/bamfile');
 ...
 my $index = QCMG::SamTools::Bam->index_open('/path/to/bamfile');
 my $index = QCMG::SamTools::Bam->index_open_in_safewd('/path/to/bamfile');
...
B<Access to the reference sequence> When you create the QCMG::SamTools::Sam
 ...
 * QCMG::SamTools::Sam               -- A collection of alignments and reference sequences.
 * QCMG::SamTools::Bam::Alignment    -- The alignment between a query and the reference.
 * QCMG::SamTools::Bam::Query        -- An object corresponding to the query sequence in
                                  which both (+) and (-) strand alignments are
                                  shown in the reference (+) strand.
 * QCMG::SamTools::Bam::Target       -- An interface to the query sequence in which
                                   (-) strand alignments are shown in reverse
                                   complement

You may encounter other classes as well. These include:

 * QCMG::SamTools::Sam::Segment       -- This corresponds to a region on the reference
                                  sequence.
 * QCMG::SamTools::Sam::Constants     -- This defines CIGAR symbol constants and flags.
 * QCMG::SamTools::Bam::AlignWrapper  -- An alignment helper object that adds split
                                  alignment functionality. See QCMG::SamTools::Bam::Alignment
                                  for the documentation on using it.
 * QCMG::SamTools::Bam::ReadIterator  -- An iterator that mediates the one-feature-at-a-time 
                                  retrieval mechanism.
 * QCMG::SamTools::Bam::FetchIterator -- Another iterator for feature-at-a-time retrieval.
 ...
 * QCMG::SamTools::Tam            -- Methods that read and write TAM (text SAM) files.
 * QCMG::SamTools::Bam            -- Methods that read and write BAM (binary SAM) files.
 * QCMG::SamTools::Bam::Header    -- Methods for manipulating the BAM file header.
 * QCMG::SamTools::Bam::Index     -- Methods for retrieving data from indexed BAM files.
 * QCMG::SamTools::Bam::Alignment -- Methods for manipulating alignment data.
 * QCMG::SamTools::Bam::Pileup    -- Methods for manipulating the pileup data structure.
 * QCMG::SamTools::Sam::Fai       -- Methods for creating and reading from indexed Fasta
                              files.
=head1 METHODS
...
=head2 QCMG::SamTools::Sam Constructor and basic accessors

=over 4

=item $sam = QCMG::SamTools::Sam->new(%options)

The QCMG::SamTools::Sam object combines a Fasta file of the reference
...
An example of a typical new() constructor invocation is:
  ...
  $sam = QCMG::SamTools::Sam->new(-fasta => '/home/projects/genomes/hu17.fa',
...
Return the QCMG::SamTools::Bam::Header object associated with the BAM
file. You can manipulate the header using the low-level API.

=item $bam    = $sam->bam

Returns the low-level QCMG::SamTools::Bam object associated with the opened
file.

=item $fai    = $sam->fai

Returns the QCMG::SamTools::Sam::Fai object associated with the Fasta
...
Return the QCMG::SamTools::Bam::Index object associated with the BAM file. 
...
QCMG::SamTools::SAM objects are not stable across fork() operations. If you
...
The QCMG::SamTools::Sam object provides the following methods for getting
...
seqid. Note that this is a string, not a QCMG::SamTools::PrimarySeq object. If
...
QCMG::SamTools::Sam::Segment objects refer regions on the reference
...
Return a QCMG::SamTools::PrimarySeq object corresponding to the sequence of the
reference under this segment. You can get the actual DNA string in
this redundant-looking way:

 $dna = $segment->seq->seq

The advantage of working with a QCMG::SamTools::PrimarySeq object is that you can
...
QCMG::SamTools::Sam->features() method for the full list of options. Here are
...
QCMG::SamTools::Bam::AlignWrapper.
...
top-level feature is of type QCMG::SamTools::SeqFeature; it contains two
QCMG::SamTools::Bam::AlignWrapper subparts. 
...
QCMG::SamTools::Sam::Segment representing the reference sequences. These two
...
QCMG::SamTools::Bam::AlignWrapper on the argument list.
...
QCMG::SamTools::Bam::Pileup objects. Here is the typical call signature:
...
Each QCMG::SamTools::Bam::Pileup object describes the position of a read in
the alignment. Briefly, QCMG::SamTools::Bam::Pileup has the following
methods:

 $pileup->alignment  The alignment at this level (a
                     QCMG::SamTools::Bam::AlignWrapper object).
...
low-level QCMG::SamTools::Bam::Alignment objects rather than the higher-level
QCMG::SamTools::Bam::AlignWrapper objects. This makes it roughly 50% faster,
but you lose the align objects' seq_id() and get_tag_values()
methods. As a compensation, the callback receives an additional
argument corresponding to the QCMG::SamTools::Sam object. You can use this to
create AlignWrapper objects on an as needed basis:
 ...
 my $callback = sub {
    ...
    for my $p (@$pileup) {
       ...
       my $wrapper   = QCMG::SamTools::Bam::AlignWrapper->new($alignment,$sam);
...
file and is a small 0-based integer index. The QCMG::SamTools::Bam::Header
...
=item $fai = QCMG::SamTools::Sam::Fai->load('/path/to/file.fa')
...
For consistency with QCMG::SamTools::Bam->open() this method is also called
open().
...
=item $tam = QCMG::SamTools::Tam->open('/path/to/file.sam')
...
Create and return a QCMG::SamTools::Bam::Header object from the information
...
Given a QCMG::SamTools::Bam::Header object, such as the one created by
header_read2(), and a QCMG::SamTools::Bam::Alignment object created by
QCMG::SamTools::Bam::Alignment->new(), reads one line of alignment information
...
=item $bam = QCMG::SamTools::Bam->open('/path/to/file.bam' [,$mode])
...
Note that QCMG::SamTools::Bam objects are not stable across fork()
operations. If you fork, and intend to use the object in both parent
and child, you must reopen the QCMG::SamTools::Bam in either the child or the
...
Example:
   ...
   $bam = QCMG::SamTools::Bam->open('http://some.site.com/nextgen/chr1_bowtie.bam');

=item $header = $bam->header()

Given an open BAM file, return a QCMG::SamTools::Bam::Header object
containing information about the reference sequence(s).

=item $status_code = $bam->header_write($header)

Given a QCMG::SamTools::Bam::Header object and a BAM file opened in write
...
QCMG::SamTools::Bam::Alignment object.

=item $bytes = $bam->write1($alignment)

Given a BAM file that has been opened in write mode and a
QCMG::SamTools::Bam::Alignment object, write the alignment to the BAM file
and return the number of bytes successfully written.

=item QCMG::SamTools::Bam->sort_core($by_qname,$path,$prefix,$max_mem)
...
The QCMG::SamTools::Bam::Index object provides access to BAM index (.bai)
files.

=over 4

=item $status_code = QCMG::SamTools::Bam->index_build('/path/to/file.bam')
...
=item $index = QCMG::SamTools::Bam->index('/path/to/file.bam',$reindex)
...
=item $index = QCMG::SamTools::Bam->index_open('/path/to/file.bam')

Attempt to open the index file for a BAM file, returning a
QCMG::SamTools::Bam::Index object. The filename path to use is the .bam file,
not the .bai file.

=item $index = QCMG::SamTools::Bam->index_open_in_safewd('/path/to/file.bam' [,$mode])
...
repeatedly invoke the coderef with each QCMG::SamTools::Bam::Alignment
...
Arguments:
 ...
 $bam          The QCMG::SamTools::Bam object that corresponds to the
               index object.

 $tid          The target ID of the reference sequence. This can
               be obtained by calling $header->parse_region() with
               an appropriate opened QCMG::SamTools::Bam::Header object.
...
The first argument is a QCMG::SamTools::Bam::Alignment object. The second is
...
position on the alignment, an arrayref of QCMG::SamTools::Bam::Pileup
...
The QCMG::SamTools::Bam::Pileup object was described earlier in the
...
The QCMG::SamTools::Bam::Header object contains information regarding the
...
reading from a .fai file with the QCMG::SamTools::Tam->header_read2() method,
or by reading from a BAM file using QCMG::SamTools::Bam->header(). You can
also create header objects from scratch, although there is not much
that you can do with such objects at this point.

=over 4

=item $header = QCMG::SamTools::Bam::Header->new()
...
This method will accept a QCMG::SamTools::Bam::Alignment object, convert it
...
=head2 QCMG::SamTools::Bam::Pileup methods

An array of QCMG::SamTools::Bam::Pileup object is passed to the pileup()
...
Return the QCMG::SamTools::Bam::Alignment object at this level. This provides
...
Please see L<QCMG::SamTools::Bam::Alignment> for documentation of the
QCMG::SamTools::Bam::Alignment and QCMG::SamTools::Bam::AlignWrapper objects.
...
use Carp 'croak';
use QCMG::SamTools::SeqFeature;
use QCMG::SamTools::PrimarySeq;

use base 'DynaLoader';
bootstrap QCMG::SamTools::Sam;

use QCMG::SamTools::Bam::Alignment;
use QCMG::SamTools::Sam::Segment;
use QCMG::SamTools::Bam::AlignWrapper;
use QCMG::SamTools::Bam::PileupWrapper;
use QCMG::SamTools::Bam::FetchIterator;
use QCMG::SamTools::Bam::ReadIterator;
...
sub new {
    ...
    my $bam = QCMG::SamTools::Bam->open($bam_path)      or croak "$bam_path open: $!";
...
sub clone {
    my $self = shift;
    $self->{bam} = QCMG::SamTools::Bam->open($self->{bam_path})     if $self->{bam_path};
...
sub new_dna_accessor {
	...
	my $a = QCMG::SamTools::Sam::Fai->open($accessor)  or croak "$accessor open: $!"
...
sub fetch {
    ...
    my $code     = sub {
	my ($align,$self) = @_;
	$callback->(QCMG::SamTools::Bam::AlignWrapper->new($align,$self));
    };
...
sub pileup {
	...
	my @p = map {
	      QCMG::SamTools::Bam::PileupWrapper->new($_,$self)
	      } @$pileup;
...
sub segment {
    ...
    return QCMG::SamTools::Sam::Segment->new($self,$seqid,$start,$end);
}
...
sub features {
	...
	if ($iterator && !$use_index) {
	    ...
	    die $@ if $@;
	    return QCMG::SamTools::Bam::ReadIterator->new($self,$self->{bam},$code);
	}
    ...
    return $iterator ? QCMG::SamTools::Bam::FetchIterator->new(\@result,$self->last_feature_count)
	             : @result;
}
...
sub _push_features {
    ...
    return 'push @result,QCMG::SamTools::Bam::AlignWrapper->new($a,$self)'
	unless $max;
    ...
    if (\@result < $max) {
	push \@result,QCMG::SamTools::Bam::AlignWrapper->new(\$a,\$self);
    } else {
	\$result[rand \@result] = QCMG::SamTools::Bam::AlignWrapper->new(\$a,\$self) 
	    if rand() < $max/\$count;
    }
END
    return $code;
...
sub bam_index {
    my $self = shift;
    return $self->{bai} ||= QCMG::SamTools::Bam->index($self->{bam_path},$self->autoindex);
}
...
sub _filter_by_attribute {
	...
	for my $c (@comps) {
		...
		push @matches,"length \$value && \$value == $c";
	    }
...
#use base 'QCMG::SamTools::SeqFeature';
...
sub index {
    ...
    return $self->index_open_in_safewd($path) if QCMG::SamTools::Sam->is_remote($path);
...
The QCMG::SamTools::Sam interface can be used as a backend to GBrowse
...
L<Bio::Perl>, L<QCMG::SamTools::Bam::Alignment>, L<QCMG::SamTools::Bam::Constants>
