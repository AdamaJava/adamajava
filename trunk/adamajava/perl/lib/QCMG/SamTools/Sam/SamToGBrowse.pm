package QCMG::SamTools::Sam::SamToGBrowse;
use Carp 'croak';
use File::Spec;
...
sub sort_bam {
    ...
    QCMG::SamTools::Bam->sort_core(0,$bam,$sorted);
...
sub _fork_and_index_bam {
    ...
    QCMG::SamTools::Bam->index_build($bam);
    exit 0;
}
sub dir_path {
    my $self = shift;
...
sub write_coverage {
    ...
    my $bam = QCMG::SamTools::Sam->new(-bam=>$bamfile) 
...
sub make_bigwig_file {
	...
	QCMG::SamTools::BigFile->createBigWig($infile,$chrom_sizes,$outfile);
    }
}
...
[${track}_bw:database]
db_adaptor = QCMG::SamTools::BigWig
db_args    = sub { require QCMG::SamTools::Sam;
                   return ( 
                       -bigwig => '$bw',
		       -fasta  => QCMG::SamTools::Sam::Fai->open('$fasta'),
		       );
                 }
