...
=head1 USAGE
 ...
 perl XMate.pl -c <configFile>

=head1 DESCRIPTION

This script is the manager script for the X-MATE pipeline.
For detailed description of this pipeline please see the
user manual.
...
$objTool->Log_PROCESS("welcome to our X-MATE");

# create tag file for recurisve mapping, the number of tag files are the number of recursive loops
my $f_forMap;
$f_forMap = $objTool->Pre_recursiveMap(
    $objConf->output_dir,  $objConf->exp_name,   $objConf->raw_csfasta,
    $objConf->map_lengths, $objConf->space_code, $objConf->map_ISAS);

#  determine the mapping strategy and initialise the run mapper.
my $objMap;
if ($objConf->map_ISAS eq TRUE) {
    if ($objConf->map_junction eq TRUE) {
        $objMap = QCMG::X_Mate::RNA_ISAS->new('conf' => $objConf);
    }
    else { $objMap = QCMG::X_Mate::DNA_ISAS->new('conf' => $objConf) }
}
else {
    if ($objConf->map_junction eq TRUE) {
...
# select single mapping tags, or rescue multi mapping tags.
my $f_positive;
my $f_negative;
if ($objConf->run_rescue eq TRUE) {
    ...
    ($f_positive, $f_negative) = &SingleSelect($objConf, $objMap->genome_collated);
}
...
#  create a wiggle file for unstranded data.
sub nonStrandWig {
    my ($objConf, $ref_positive, $ref_negative) = @_;

    # join both positive and negative file into one, named positive for now
    for (my $i = 0 ; $i < scalar(@$ref_positive) ; $i++) {
        ...
        open(NEG, $fn)
          or $objConf->Log_DIED("can't open file $fn in sub nonStrandWig");
    ...
    my $objWig = QCMG::BaseClass::WigStart->new(
        ...
        'output_dir' => $objConf->output_dir,
        'objTool'    => $objTool);
    $objWig->main($ref_positive, "wiggle", $objConf->exp_name.".unstranded", $objConf->exp_name . " - unstranded");
    $objWig->main($ref_positive, "start", $objConf->exp_name.".unstranded", $objConf->exp_name . " - unstranded");
...
#  create wiggle files for stranded data
sub StrandWig {
    my ($objConf, $ref_positive, $ref_negative) = @_;
    ...
    # positive tags
    $objWig->main($ref_positive, "wiggle", $objConf->exp_name.".stranded.pos", $objConf->exp_name . " - positive strand");
    rename($objWig->output, $objWig->output . ".positive");
    $objWig->main($ref_positive, "start", $objConf->exp_name.".stranded.pos", $objConf->exp_name . " - positive strand");
    rename($objWig->output, $objWig->output . ".positive");

    # negative tags
    $objWig->main($ref_negative, "wiggle", $objConf->exp_name.".stranded.neg", $objConf->exp_name . " - negative strand");
    rename($objWig->output, $objWig->output . ".negative");
    $objWig->main($ref_negative, "start", $objConf->exp_name.".stranded.neg", $objConf->exp_name . " - negative strand");
    rename($objWig->output, $objWig->output . ".negative");
...
sub SingleSelect {
    ...
    my $objSiM = QCMG::BaseClass::SingleSelect->new(
        ...
        # must set "0" to strand, since we ignore strand for genome mapping selection
        'expect_strand' => "0",
