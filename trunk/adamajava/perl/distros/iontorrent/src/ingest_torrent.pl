...
#
# $Id: ingest_torrent.pl 4668 2014-07-24 10:18:42Z j.pearson $
#
##############################################################################
...
if($USAGE || (! $RUN_FOLDER || ! $LOG_FILE) ) {
	...
	Ingest raw sequencing Ion Torrent files into Mediaflux
...
my $qi		= QCMG::Ingest::Torrent->new(RUN_FOLDER => $RUN_FOLDER, LOG_FILE => $LOG_FILE);
