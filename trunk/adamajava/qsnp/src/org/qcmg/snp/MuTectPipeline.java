...
//	private final ConcurrentMap<ChrPosition, QSnpRecord> mapQsnps = new ConcurrentHashMap<ChrPosition, QSnpRecord>();
...
//		if (vcfNormalFile.isEmpty()) throw new SnpException("No normal vcf data - empty file??");
//		
...
//		if (tumourVCFMap.isEmpty()) throw new SnpException("No tumour vcf data - empty file??");
	...
	private void addNovelStarts() {
		...
		try (SAMFileReader reader = SAMFileReaderFactory.createSAMFileReader(tumourBam)) {
			...
			for (Entry<ChrPosition, QSnpRecord> entry : positionRecordMap.entrySet()) {
				...
				int nsCount = acc.getNovelStartsCountForBase(rec.getAlt());
...
//	}
	...
	public static QSnpRecord getQSnpRecord(String [] mtData) {
		...
		rec.setChromosome(mtData[0]);
		rec.setPosition(Integer.parseInt(mtData[1]));
		rec.setRef(ref);
		rec.setAlt(alt);
		rec.setMutation(ref + ">" + alt);
