...
public class QSVParameters {
    ...
    private Integer compareClusterSize;
	...
	private boolean runSoftClipAnalysis;
	private List<String> readGroupIds = new ArrayList<String>();
	private File clippedBamFile;
	private int repeatCountCutoff;
	...
	public QSVParameters(Options options, boolean isTumor, String masterDirPath, String matePairFileDir,
        ...
        if (options.runPairPreprocess()) {	       
	        String baseName = inputBamFile.getName();
	        String fileName = baseName.replace(".bam", ".discordantpair.filtered.bam");
        ...
        if (options.runClipPreprocess() || options.isSplitRead()) {        	
        	String baseName = this.inputBamFile.getName();         	
	...
	private void getISizesFromIniFile(Options options) throws Exception {
		...
		Ini iniFile = new Ini(new File(options.getIniFile()));				
		
		Set<Entry<String, Section>> sectionList = iniFile.entrySet();
			...
			if (section.getParent() != null) {				
				if (section.getParent().getName().equals(parent)) {	
					if (section.containsKey("lower") && section.containsKey("upper") && section.containsKey("rgid")) {				
						...
						RunTypeRecord runRecord = new RunTypeRecord(section.get("rgid"), new Integer(section.get("lower")), new Integer(section.get("upper")), name);
...
//			this.sequencingRuns = est.getRunRecords();
			...
			if (options.runClipAnalysis() && !options.runPairAnalysis()) {
				...
				SAMFileReader reader = SAMFileReaderFactory.createSAMFileReader(inputBamFile, "silent");
				for (SAMReadGroupRecord record: reader.getFileHeader().getReadGroups()) {
				...
				for (RunTypeRecord r: sequencingRuns) {
				...
				for (RunTypeRecord record : sequencingRuns) {
	...
	private void getChromosomesToAnalyse(Options options) throws QSVException {
		...
		inputSam = SAMFileReaderFactory.createSAMFileReader(inputBamFile, "silent"); 
        ...
        if (options.allChromosomes() || options.getIncludeTranslocations()) {        	
			for (SAMSequenceRecord seq : sequenceRecords) {
				String key = seq.getSequenceName();				
				 Chromosome c = new Chromosome(key, seq.getSequenceLength());
        	...
        	for (String r : options.getRanges()) {
        		//subsection of a chromosome
	        	if (r.contains(":")) {
					String chrName = r.split(":")[0];
					String pos = r.split(":")[1];
					Integer start = new Integer(pos.split("-")[0]);
					Integer end = new Integer(pos.split("-")[1]);
					 for (SAMSequenceRecord seq : sequenceRecords) {
			        	 if (seq.getSequenceName().equals(chrName)) {
			        		 Chromosome c = new Chromosome(chrName, seq.getSequenceLength(), start, end);
					...
					for (SAMSequenceRecord seq : sequenceRecords) {
			        	 if (seq.getSequenceName().equals(chrName)) {
			        		 Chromosome c= new Chromosome(chrName, seq.getSequenceLength());
			        		 if (chromosomes.containsKey(chrName)) {
			        			 chromosomes.get(chrName).add(c);
			        		 } else {
			        			 List<Chromosome> list = new ArrayList<Chromosome>();
	...
	public String getISizeReportString() {
		...
		for (int i=0; i< sequencingRuns.size(); i++) {
			RunTypeRecord r = sequencingRuns.get(i);
